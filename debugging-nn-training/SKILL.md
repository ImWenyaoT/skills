...
| 若涉及 Azure ML / PyTorch on Azure 训练 | `microsoft_docs_search(query="Azure Machine Learning PyTorch train debug")` |

## CLI Alternative

若 Learn MCP server 不可用（仅当确实涉及 Microsoft/Azure 主题时需要），可改用 `mslearn` CLI：
...
直接运行 `npx @microsoft/learn-cli <command>`，或全局安装 `npm install -g @microsoft/learn-cli`。
