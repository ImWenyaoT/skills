...
JD 驱动流程（`prompts/jd-driven-prep.md` Step 7）的可选 HTML 报告规范。仅在用户需要文件时使用，不依赖其他技能的模板。

## 双语输出（可选）

只有用户或工作区要求双语时，才加入中文 / EN 切换按钮，并用 CSS 根据 `<html lang>` 切显 `[data-lang="en"]` 和 `[data-lang="zh"]`：
...
- 路径遵循工作区配置；未配置时写入当前工作区。
- 优先单文件和本地字体栈，避免把网络字体作为正确渲染的必要条件。
...
CJK 字体顺序遵循工作区配置，并在 PDF 导出后检查实际嵌入结果。
```
