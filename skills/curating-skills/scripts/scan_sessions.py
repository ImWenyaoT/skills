...
用法（在本 skill 目录下运行，源仓库与已安装副本都适用）:
    uv run python scripts/scan_sessions.py                 # 盘点 codex+claude，过滤 guardian
    uv run python scripts/scan_sessions.py --codex         # 只看 codex
    uv run python scripts/scan_sessions.py --claude        # 只看 claude 真实 transcript
    uv run python scripts/scan_sessions.py --dump <子串>   # 打印某个会话的干净对话（按文件名子串匹配）
