...
description: Use when modifying agent persistence — SQLite/DB repositories, session state, memory snapshots, trace persistence, JSON-fallback migration, or eval/debug trace fields. Triggers on memory store, trace rows, session state, or migrating agent data from JSON to a database.
---
