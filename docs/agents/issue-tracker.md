...
External PRs are not processed as triage requests. `/triage` should use GitHub Issues as the request queue and leave PRs to normal code-review workflows.

If this changes later, set the value above to `yes`. Then PRs run through the same labels and states as issues, using the `gh pr` equivalents:
